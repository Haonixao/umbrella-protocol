...
// CloseWrite реализует интерфейс closeWriter, позволяя закрыть поток записи
// отдельно от потока чтения.
func (c *Conn) CloseWrite() error {
	if c.writer != nil {
		// В нашем случае writer — это либо StreamWriter, либо PacketUpWriter.
		// У обоих есть метод Close(), который закроет подлежащий io.PipeWriter.
