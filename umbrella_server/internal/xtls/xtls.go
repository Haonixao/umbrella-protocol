...
func handleVisionUDP(stream net.Conn) error {
	if _, err := stream.Write([]byte{0x00}); err != nil {
		return fmt.Errorf("write Vision UDP ack: %w", err)
	}

	pc, err := net.ListenPacket("udp", "0.0.0.0:0")
	if err != nil {
		return fmt.Errorf("Vision UDP listen: %w", err)
	}
	defer pc.Close()
	...
	for {
		...
		if err := visionWriteDatagram(stream, frameBuf[:off]); err != nil {
			return fmt.Errorf("Vision UDP relay stream write: %w", err)
		}
	}
}
...
func handleVisionTunnel(conn net.Conn) error {
	...
	if err != nil {
		conn.Write([]byte{0x01})
		return fmt.Errorf("vision dial %s failed: %w", target, err)
	}
	defer remote.Close()
